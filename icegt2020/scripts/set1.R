...
axis_seq <- as.character(seq.Date(as.Date(startdate), as.Date(enddate), "day"))
...
index_col <- c(yellow, blue, yellow, blue, yellow, blue, yellow, blue)
...
# plotting a pdf
jpeg(paste0(file_loc, "fig_set1.jpeg"),
...
# first subplot
par(mar = c(0.25, 4.25, 0.25, 0.25), mgp = c(0.1, 0.1, 0),
  family = "sans", ps = 14, cex = 1, cex.main = 1, las = 1)
plot(0, 0, xlim = c(startdate, enddate), ylim = c(15, 30),
  type = "l", axes = F, xlab = NA, ylab = NA)
for (i in 1:3) {
...
par(las = 0)
mtext(expression(paste("Heat flux @ 940 mm [W/m"^2, "]")), side = 2, line = 2)
for (i in 1:3) {
