...
# plotting a pdf file
jpeg(paste0(file_loc, "fig_set2.jpeg"),
...
legend("center", c("Soil temperature", "Heat flux"),
       lty = 1, col = c(red, yellow), hor = T, bty = "n", lwd = 2)
# first subplot
par(mar = c(2, 3.25, 0.25, 3.25), mgp = c(0.1, 0.1, 0),
...
box()
par(las = 0)
mtext("Soil temperature @ 850 mm [°C]", side = 2, line = 2.25)
mtext("Time [DD-MM-2019]", side = 1, line = 1)

lines(soil_temp_set2[, 13]~utc(soil_temp_set2$Time), lwd = 2, col = red)
par(new = T, las = 1)
...
mtext("Depth[mm]", side = 2, line = 2.25)
dev.off()