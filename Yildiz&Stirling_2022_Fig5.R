...
soil_temp_set1 <- soil_temp[which(as.POSIXct(soil_temp[,1],"UTC") > startdate-1 &
                                    as.POSIXct(soil_temp[,1],"UTC") < enddate),]
meteo_set1 <- meteo[which(as.POSIXct(meteo[,1],"UTC") > startdate-1 &
                            as.POSIXct(meteo[,1],"UTC") < enddate),]
heat_flux_set1 <- heat_flux[which(as.POSIXct(heat_flux[,1],"UTC") > startdate-1 &
...
{
  points(soil_temp_set1[seq(1,nrow(soil_temp_set1),50),index.column[i]]~as.POSIXct(soil_temp_set1[seq(1,nrow(soil_temp_set1),50),1],"UTC"),
         col=index.col[i],pch=index.pch[i],cex=0.8) 
  lines(soil_temp_set1[,index.column[i]]~as.POSIXct(soil_temp_set1[,1],"UTC"),lwd=2,col=index.col[i],lty=index.lty[i])
}
...
for(i in 1:3)
{
  ...
  text(as.POSIXct(dates[i,2],"UTC")+60*60*48,30,paste0("Off"),adj=c(0.5,1),font=2)
}
...
axis(2,tck=0.02)
box()
lines(heat_flux_set1$HF.Bottom~as.POSIXct(heat_flux_set1[,1],"UTC"),lwd=2)
par(las=0)
mtext(expression(paste("Heat flux @ 940 mm [W/m"^2,"]")),side=2,line=1)
for(i in 1:3)
{
  text(as.POSIXct(dates[i,1],"UTC")+60*60*36,35,paste0("Test I-",i),adj=c(0.5,1),font=2)
  text(as.POSIXct(dates[i,2],"UTC")+60*60*48,35,paste0("Off"),adj=c(0.5,1),font=2)
}
