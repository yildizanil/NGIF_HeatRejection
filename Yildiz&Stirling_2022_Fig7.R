...
{
  mape_data[k] <- mape(y_observed = data_observed[,k],y_predicted = data_predicted[,k])
  rmse_data[k] <- rmse(y_observed = data_observed[,k],y_predicted = data_predicted[,k],normalise = T)
  r2_data[k] <- r2(y_observed = data_observed[,k],y_predicted = data_predicted[,k])
}
...
plot(0,0,xlim=c(0,0),ylim=c(10,30),type="l",axes=F,xlab=NA,ylab=NA)
legend("center",c("Measured","Estimated"),lwd=2,lty=c(3,1),col=c(1,blue),hor=T,bty="n")
for(i in 1:8)
{
  par(mar=c(0.25,2,0.25,0.5),mgp=c(0.1,0.1,0),family="serif",ps=10,cex=1,cex.main=1,las=1)
  plot(0,0,xlim=c(startdate,enddate),ylim=c(10,30),type="l",axes=F,xlab=NA,ylab=NA)
  segments(x0=seq(startdate,enddate,60*60*24),y0=10,x1=seq(startdate,enddate,60*60*24),y1=30,col="gray87")
  segments(x0=startdate,y0=seq(10,30,5),x1=enddate,y1=seq(10,30,5),col="gray87")
  ...
  lines(data_predicted[,index[i]]~utc(data_observed$Time),lwd=1,col=blue,lty=1)
  lines(data_observed[,index[i]]~utc(data_observed$Time),lwd=2,col=1,lty=3)
  
  text(enddate,30,paste0("nRMSE=",round(rmse_data[index[i]],2), "%\n","MAPE=",round(mape_data[index[i]],2), "%"),adj=c(1,1))
